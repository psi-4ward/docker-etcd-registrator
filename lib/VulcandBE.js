...
VulcandBE.prototype._buildBeServerUrl = function _buildBeServerUrl(portService) {
  ...
  debug('Ignore ' + portService.name + ':' + portService.port + ' no VULCAND_BE attribs');
  return false;
...
VulcandBE.prototype._buildFeCatalogUrl = function _buildFeCatalogUrl(portService) {
  ...
  debug('Ignore ' + portService.name + ':' + portService.port + ' no VULCAND_FE attribs');
  return false;
};
 ...
 * @param {String} cid
 */
VulcandBE.prototype.removeServiceByCid = function removeServiceByCid(cid) {
  var self = this;
...
function tolerantDel(url, cb) {
  etcd.del(url, function(err, retData, retHeader) {
    if(err) console.error('Error: Could not delete ' + err.error.cause + ': ' + err.error.message);
    cb(null, retData, retHeader);
  });
}
...
VulcandBE.prototype.removeByUrls = function removeByUrls(urls, cb) {
  ...
  async.eachLimit(urls, 5, function(url, next) {
    ...
    if(type === 'servers' || type === 'registrator-catalog') {
      debug('Remove: ' + url);
      async.waterfall([
        tolerantDel.bind(this, url),
        function isEmpty(delret, delheader, cb) {
          etcd.get(urlBase + '/' + type, function(err, node) {
            if(err && err.errorCode === 100) return cb(null, true);
            if(err) return cb(err);
            cb(null, !(node && node.node && node.node.nodes && node.node.nodes.length > 0));
          });
        },
        function(isEmpty, cb) {
          if(!isEmpty) return cb();
          debug(urlBase + '/' + type + ' is empty, removing ' + urlBase);
          etcd.del(urlBase, {recursive: true}, cb);
        }
      ], function(err) {
        if(err) console.error('Error: Could not delete ' + err.error.cause + ': ' + err.error.message);
        next();
...
VulcandBE.prototype.sync = function (activeServicesByPort, cb) {
  ...
  etcd.get(this.prefix, {recursive: true}, function(err, obj) {
    ...
    function addRunning(cb) {
      ...
      async.eachLimit(toAdd, 10, function(url, next) {
        self.addService(runningMap[url], function() {
